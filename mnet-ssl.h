...
public:
    virtual OnRead( SSLSocket* , std::size_t , const NetState& ) = 0;
...
public:
    virtual OnWrite( SSLSocket* , std::size_t , const NetState& ) = 0;
...
private:
    ...
    bool DoBufferRecv( NetState* state );
...
void SSLSocket::AsyncRead<T>( T* notifier ) {
    assert( state_ == SOCKET_CONNECTED ); // Idel status
    assert( ssl_io_stat e_ == DONE ); // Idel status
    bool write;
    std::size_t size;
    NetState state;
    int ssl_state = DoSSLReadLoop( &state , &write , &size );
    if( !state ) {
        // This means we have some problem that we cannot resolve 
        notifier->OnRead( this, size , state );
        return;
    } else {
        if( ssl_state == DONE ) {
            // We have finished the IO operation just call user's callback
            notifier->OnRead( this, size , state );
            return;
        } else {
            DoUnderlyIO(true,write);
            read_callback_.Reset( detail::MakeSSLReadCallback(notifier) );
            state_ = SOCKET_READING;
            ssl_io_state_ = ssl_state;
        }
    }
}
...
void SSLSocket::AsyncWrite<T>( T* notifier ) {
    assert( state_ == SOCKET_CONNECTED );
    if( ProcessIO(&DoSSLWriteLoop) == Done ) {
    }
}
...
void SSLSocket::AsyncClose( T* notifier ) {
    assert( state_ == SOCKET_CONNECTED );
    if( ProcessIO(&DoSSLShutdownLoop) == Done ) {
    }
}
